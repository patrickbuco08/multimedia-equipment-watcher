    ...
    if (empty($equipment_id) || empty($report_date) || empty($description)) {
        ...
        try {
            ...
            if (isset($_FILES['lost_image']) && $_FILES['lost_image']['error'] === UPLOAD_ERR_OK) {
                ...
                $file_extension = strtolower(pathinfo($_FILES['lost_image']['name'], PATHINFO_EXTENSION));
                $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif'];
                
                if (in_array($file_extension, $allowed_extensions)) {
                    $new_filename = 'lost_' . time() . '_' . uniqid() . '.' . $file_extension;
            ...
            // Insert report
            $stmt = $pdo->prepare("INSERT INTO equipment_reports (equipment_id, report_type, reported_by, report_date, quantity, description, image_path) VALUES (?, 'lost', ?, ?, ?, ?, ?)");
            $stmt->execute([$equipment_id, $_SESSION['user_id'], $report_date, $quantity, $description, $image_path]);
            
            $report_id = $pdo->lastInsertId();
            
            // Update equipment status to lost
            $stmt = $pdo->prepare("UPDATE equipment SET status = 'lost' WHERE id = ?");
...
<div class="max-w-3xl bg-white rounded-lg shadow-sm p-6">
    ...
    <form method="POST" action="" enctype="multipart/form-data" class="space-y-5">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                <label for="report_date" class="block text-sm font-medium text-gray-700 mb-2">The Possible Date It Was Lost *</label>
        ...
        <div>
            <label for="description" class="block text-sm font-medium text-gray-700 mb-2">Detailed Description of the Lost Item *</label>
        ...
        <div>
            <label for="lost_image" class="block text-sm font-medium text-gray-700 mb-2">Upload Image (Optional)</label>
            <input type="file" id="lost_image" name="lost_image" accept="image/*" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
            <p class="text-xs text-gray-500 mt-1">Accepted formats: JPG, JPEG, PNG, GIF</p>
        </div>
        
        <div class="flex gap-3 pt-4">
            <button type="submit" class="px-6 py-3 bg-orange-600 hover:bg-orange-700 text-white font-semibold rounded-lg transition" <?php echo count($equipment) === 0 ? 'disabled' : ''; ?>>Submit Report</button>
